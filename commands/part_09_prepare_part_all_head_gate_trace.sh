...
cat > tools/part_all_head_gate_trace_real_contract_v1.py <<'PY'
#!/usr/bin/env python3
import argparse, csv, json, math, gc
...
cat > commands/part_09_all_head_gate_trace_real_contract.sh <<'SH'
#!/usr/bin/env bash
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"
...
SH
chmod +x tools/part_all_head_gate_trace_real_contract_v1.py commands/part_09_all_head_gate_trace_real_contract.sh
bash commands/part_09_all_head_gate_trace_real_contract.sh
