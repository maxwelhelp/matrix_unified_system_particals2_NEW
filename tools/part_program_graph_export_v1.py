...
def main():
    ...
    logit=add_node(nodes,'objective','Tbl-Hqql','J=logit_Tbl-logit_Hqql')

    # classifier nodes and edges
    fc=add_node(nodes,'classifier','mod.fc','classifier head')
    add_edge(edges,fc,logit,'classifier_to_objective',1.0,formula='logits=fc(z)')
    for r in sorted(lin,key=lambda r:abs(f(r.get('weight_tbl_minus_hqql'))),reverse=True)[:24]:
        dim='fc_dim_'+str(r.get('dim'))
        dn=add_node(nodes,'classifier_dim',dim,dim,W_Tbl_minus_Hqql=r.get('weight_tbl_minus_hqql'),bias_Tbl_minus_Hqql=r.get('bias_tbl_minus_hqql'))
        add_edge(edges,dn,fc,'linear_direction',f(r.get('weight_tbl_minus_hqql')),dim=r.get('dim'))
    ...
    obj={'ok':True,'nodes':len(node_rows),'edges':len(edges),'node_kinds':dict(bykind),'edge_kinds':dict(byedge),'top_validated_pairs':valid[:20]}
    ...
    lines.append(mdtab(['rank','source','target','W_Tbl-Hqql','dim'],[[i+1,e['source'],e['target'],fmt(e['weight']),e.get('dim')] for i,e in enumerate(le)]))
    ...
    print(json.dumps({'ok':True,'nodes':len(node_rows),'edges':len(edges),'out_md':a.out_md},indent=2))
