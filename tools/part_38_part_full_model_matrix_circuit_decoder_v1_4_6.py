#!/usr/bin/env python3
"""
PART full-model matrix coverage decoder v1_4_6.
...
def decode_conv1d(name: str, module: nn.Conv1d, rec: Optional[Dict[str, Any]], tensor_root: Path, args: argparse.Namespace) -> Tuple[Dict[str, Any], List[Dict[str, Any]], List[Dict[str, Any]]]:
    """Exact Conv1d replay for pair_embed / conv modules.

    Covers weight/bias as an exact constant convolution operator:
      y = F.conv1d(x, W, b, stride, padding, dilation, groups)
    For kernel_size=1 it also exposes W[:,:,0] as a matrix target for primitive fitting.
    """
    if rec is None or rec.get("input") is None or rec.get("output") is None:
        return uncovered_module_row(name, module, "missing_capture"), [], []

    x = rec["input"].detach().float().cpu()
    y = rec["output"].detach().float().cpu()
    W = module.weight.detach().float().cpu()
    b = module.bias.detach().float().cpu() if module.bias is not None else None

    def run_conv(dtype=torch.float32, device="cpu"):
        xx = x.to(device=device, dtype=dtype)
        ww = W.to(device=device, dtype=dtype)
        bb = None if b is None else b.to(device=device, dtype=dtype)
        padding_arg = module.padding
        # nn.Conv1d with padding_mode != "zeros" pads before F.conv1d.
        # F.conv1d itself only implements zero padding. Without this branch,
        # exact replay silently fails for reflect/replicate/circular padding.
        if getattr(module, "padding_mode", "zeros") != "zeros":
            if isinstance(module.padding, str):
                # PyTorch Conv1d does not combine non-zero padding_mode with
                # string padding in normal use, but do not fake exactness.
                raise RuntimeError(f"unsupported Conv1d padding_mode={module.padding_mode} with padding={module.padding}")
            pad_val = module.padding[0] if isinstance(module.padding, tuple) else int(module.padding)
            if pad_val:
                xx = F.pad(xx, (pad_val, pad_val), mode=module.padding_mode)
            padding_arg = 0
        with torch.no_grad():
            yy = F.conv1d(xx, ww, bb, stride=module.stride, padding=padding_arg, dilation=module.dilation, groups=module.groups)
        return yy.detach().float().cpu()

    pred = run_conv(torch.float32, "cpu")
    err = rel_err(pred, y)
    replay_dtype = "cpu_float32"
    # ParT may run under autocast. If CPU float32 differs from captured CUDA AMP output,
    # try a CUDA fp16 replay and keep the lower error.
    if err >= args.accept_tol and torch.cuda.is_available():
        try:
            pred16 = run_conv(torch.float16, "cuda")
            err16 = rel_err(pred16, y)
            if err16 < err:
                pred, err, replay_dtype = pred16, err16, "cuda_float16_autocast_approx"
        except Exception:
            pass

    # Matrix primitive fit only for 1x1 conv. For larger kernels exact conv is still covered,
    # but primitive matrix fit is skipped because the operator is Toeplitz over sequence length.
    fit = {"enabled": False, "fit_kind": "CONV1D_NON_1X1_EXACT_OPERATOR", "rec_err": "", "selected_terms": [], "uses_target_mined_atoms": False}
    matrix_for_fit = None
    if W.ndim == 3 and W.shape[-1] == 1 and module.groups == 1:
        matrix_for_fit = W[:, :, 0]
        fit = decode_matrix_with_auto_dictionary(matrix_for_fit, allow_target_mined=args.auto_mined_atoms, max_atoms=args.decode_topk, stop_rel=args.decode_stop_rel, mined_svd_atoms=args.mined_svd_atoms, ridge=args.decode_ridge)

    tensor_path = ""
    if args.save_tensors:
        tensor_path = save_pt(str(tensor_root / "conv1d" / f"{safe_name(name)}.pt"), {
            "W": W, "b": b, "fit": fit, "matrix_for_fit": matrix_for_fit,
            "input_shape": tuple(x.shape), "output_shape": tuple(y.shape),
            "stride": module.stride, "padding": module.padding, "dilation": module.dilation,
            "groups": module.groups, "padding_mode": getattr(module, "padding_mode", "zeros"), "replay_dtype": replay_dtype,
        })

    interpretation_trusted = bool(fit.get("fit_kind", "").startswith("SPARSE_OMP") and fit.get("rec_err", 999) != "" and float(fit.get("rec_err", 999.0)) < 0.15 and not fit.get("uses_target_mined_atoms", False))
    row = {
        **common_module_fields(name, module, rec),
        "decoder_kind": "EXACT_CONSTANT_CONVOLUTION",
        "reconstruction_error": err,
        "accepted_exact": bool(err < args.accept_tol),
        "fail_reason": "" if err < args.accept_tol else f"conv1d_rel={err:.3e}",
        "tensor_path": tensor_path,
        "primitive_fit_rel_err": fit.get("rec_err", ""),
        "primitive_fit_kind": fit.get("fit_kind", ""),
        "uses_target_mined_atoms": fit.get("uses_target_mined_atoms", False),
        "uses_shared_mined_atoms": False,
        "interpretation_trusted": interpretation_trusted,
        "honesty_note": "Exact Conv1d replay. Primitive fit only for 1x1 conv matrix W[:,:,0]; larger kernels are exact conv operators, not simple channel matrices.",
        "extra_json": json.dumps({
            "fit": strip_tensors(fit), "W_shape": list(W.shape),
            "matrix_for_fit_shape": list(matrix_for_fit.shape) if matrix_for_fit is not None else [],
            "stride": module.stride, "padding": module.padding, "dilation": module.dilation,
            "groups": module.groups, "padding_mode": getattr(module, "padding_mode", "zeros"), "replay_dtype": replay_dtype,
        }, ensure_ascii=False),
    }
    ...
    if matrix_for_fit is not None:
        prim_rows.append(primitive_row(f"{name}.Conv1dW", name, "conv1d", matrix_for_fit, fit, tensor_path))
        if torch.is_tensor(fit.get("residual_after_base")):
            residual_records.append({"record_id": f"{name}.Conv1dW", "matrix": matrix_for_fit, "base_rec_err": fit.get("base_rec_err", fit.get("rec_err", "")), "residual_after_base": fit["residual_after_base"]})
    return row, prim_rows, residual_records
...
def decode_batchnorm(name: str, module: nn.Module, rec: Optional[Dict[str, Any]], tensor_root: Path, args: argparse.Namespace) -> Dict[str, Any]:
    if rec is None or rec.get("input") is None or rec.get("output") is None:
        return uncovered_module_row(name, module, "missing_capture")
    # Capture stores tensors on CPU; replay BatchNorm on CPU with copied state.
    x = rec["input"].detach().float().cpu()
    y = rec["output"].detach().float().cpu()
    weight = module.weight.detach().float().cpu() if getattr(module, "weight", None) is not None else None
    bias = module.bias.detach().float().cpu() if getattr(module, "bias", None) is not None else None
    running_mean = module.running_mean.detach().float().cpu() if getattr(module, "running_mean", None) is not None else None
    running_var = module.running_var.detach().float().cpu() if getattr(module, "running_var", None) is not None else None
    training_flag = bool(module.training or running_mean is None or running_var is None)
    momentum = 0.0 if getattr(module, "momentum", None) is None else float(module.momentum)
    with torch.no_grad():
        pred = F.batch_norm(x, running_mean, running_var, weight, bias, training=training_flag, momentum=momentum, eps=float(module.eps))
    err = rel_err(pred, y)
    tensor_path = ""
    if args.save_tensors:
        tensor_path = save_pt(str(tensor_root / "dynamic" / f"{safe_name(name)}_batchnorm.pt"), {
            "weight": weight, "bias": bias, "running_mean": running_mean, "running_var": running_var,
            "eps": float(module.eps), "training_flag_used": training_flag,
            "input_shape": tuple(x.shape), "output_shape": tuple(y.shape),
        })
    return {
        **common_module_fields(name, module, rec),
        ...
        "tensor_path": tensor_path,
        "primitive_fit_rel_err": "",
        "primitive_fit_kind": "BATCHNORM_CPU_STATE_REPLAY",
        "uses_target_mined_atoms": False,
        "uses_shared_mined_atoms": False,
        "interpretation_trusted": bool(err < args.accept_tol),
        "honesty_note": "BatchNorm replayed on CPU from copied running stats/weight/bias; no CUDA module call on CPU tensors.",
        "extra_json": json.dumps({"norm_type": module.__class__.__name__, "eps": float(module.eps), "training_flag_used": training_flag, "running_stats_available": running_mean is not None and running_var is not None}, ensure_ascii=False),
    }
...
def decode_module(name: str, module: nn.Module, rec: Optional[Dict[str, Any]], tensor_root: Path, args: argparse.Namespace) -> Tuple[Dict[str, Any], List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    if isinstance(module, nn.Conv1d):
        return decode_conv1d(name, module, rec, tensor_root, args)
    if isinstance(module, nn.Embedding):
...
def mine_shared_primitives(
    ...
    v1_4_6 important fix:
      Residual atoms from different matrix sizes MUST NOT be stacked together.
      ParT mixes 64x64, 128x128, Conv1d-flattened square matrices, etc.
      Shared mining is therefore done independently per atom_shape.
    """
    ...
    for r in residual_records:
        ...
        for c in candidate_atoms_from_residual(r["residual_after_base"], r["record_id"]):
            atom = c.get("atom")
            if not torch.is_tensor(atom) or atom.ndim != 2 or atom.shape[0] != atom.shape[1]:
                skipped_records += 1
                continue
            c["residual_record"] = r
            c["atom_shape"] = tuple(atom.shape)
            candidates.append(c)

    if not candidates:
        return [], {
            "reason": "no_residual_candidates",
            "candidates": 0,
            "skipped_records": skipped_records,
            "shape_groups": {},
            "heldout_gate": "not_run",
        }

    by_shape: Dict[Tuple[int, int], List[Dict[str, Any]]] = defaultdict(list)
    for c in candidates:
        by_shape[tuple(c["atom_shape"])].append(c)

    shared_rows: List[Dict[str, Any]] = []
    total_clusters = 0
    skipped_shape_groups: Dict[str, str] = {}

    for shape, group in sorted(by_shape.items(), key=lambda kv: (kv[0][0], kv[0][1])):
        if len(group) < min_cluster_size:
            skipped_shape_groups[str(shape)] = f"too_few_candidates:{len(group)}"
            continue

        # Safe: every atom in this group has exactly the same flattened size.
        flats = torch.stack([normalized_flat(c["atom"]) for c in group])
        used: set = set()
        ...
        for i, c in enumerate(group):
            ...
            sources = [group[j]["source"] for j in idxs]
            train_idxs = idxs[::2]
            ...
            train_atoms = [
                group[j]["atom"] / torch.linalg.norm(group[j]["atom"]).clamp_min(1e-12)
                for j in train_idxs
            ]
            ...
            train_res_gains = [
                residual_gain_for_atom(group[j]["residual_record"]["residual_after_base"], atom)
                for j in train_idxs
            ]
            heldout_res_gains = [
                residual_gain_for_atom(group[j]["residual_record"]["residual_after_base"], atom)
                for j in heldout_idxs
            ]
            ...
            for split_idxs, base_errs, shared_errs, fbase, fshared in [
                (train_idxs, train_base_errs, train_shared_errs, train_func_base, train_func_shared),
                (heldout_idxs, held_base_errs, held_shared_errs, held_func_base, held_func_shared),
            ]:
                for j in split_idxs:
                    W = group[j]["residual_record"]["matrix"]
                    # Safety: shared atom must match W shape.
                    if tuple(W.shape) != tuple(atom.shape):
                        continue
            ...
            gain_train_decode = (
                float((base_err_train - shared_err_train) / max(1e-12, base_err_train))
                if math.isfinite(base_err_train) and math.isfinite(shared_err_train)
                else 0.0
            )
            gain_heldout_decode = (
                float((base_err_heldout - shared_err_heldout) / max(1e-12, base_err_heldout))
                if math.isfinite(base_err_heldout) and math.isfinite(shared_err_heldout)
                else 0.0
            )

            base_func_heldout = avg(held_func_base)
            shared_func_heldout = avg(held_func_shared)
            gain_heldout_functional = (
                float((base_func_heldout - shared_func_heldout) / max(1e-12, base_func_heldout))
                if math.isfinite(base_func_heldout) and math.isfinite(shared_func_heldout)
                else 0.0
            )
            ...
            shared_rows.append({
                ...
                "atom_shape": "x".join(map(str, shape)),
    ...
    return shared_rows, {
        ...
        "shape_groups": {f"{k[0]}x{k[1]}": len(v) for k, v in by_shape.items()},
        "skipped_shape_groups": skipped_shape_groups,
        "skipped_records": skipped_records,
        "heldout_gate": "real_base_plus_shared_decode_gain_grouped_by_shape",
        "gain_threshold": gain_threshold,
        "decode_gain_threshold": decode_gain_threshold,
    }
...
def classify_param_kind(param_name: str, module_name: str, module: Optional[nn.Module]) -> str:
    ...
    if isinstance(module, nn.Conv1d) and param_name.endswith("weight"):
        return "conv1d_weight" if not ("pair" in low_module or "pair" in param_name.lower()) else "pair_conv1d_weight"
    if isinstance(module, nn.Conv1d) and param_name.endswith("bias"):
        return "conv1d_bias" if not ("pair" in low_module or "pair" in param_name.lower()) else "pair_conv1d_bias"
    if param_name.endswith("cls_token") or param_name.split(".")[-1] == "cls_token":
        return "learned_cls_token"
    if module is not None and module.__class__.__name__ == "Block" and param_name.split(".")[-1] in ("c_attn", "w_resid"):
        return "block_direct_gate_param"
...
def build_parameter_coverage(model: nn.Module, module_rows: List[Dict[str, Any]], head_rows: List[Dict[str, Any]], primitive_rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for param_name, p in model.named_parameters():
        ...
        # MHA out_proj child parameters are covered by parent attention head circuit.
        ...
        elif owner_module is not None and isinstance(owner_module, nn.Conv1d):
            mr = mrows.get(owner_module_name)
            prim = _primitive_for_module(primitive_rows, owner_module_name, "conv1d")
            covered = bool(mr and str(mr.get("accepted_exact")) == "True")
            decoder_row_id = f"{owner_module_name}.Conv1d"
            decoder_kind = "EXACT_CONSTANT_CONVOLUTION"
            exact_target_kind = "conv1d_exact_replay"
            exact_err = mr.get("reconstruction_error", "") if mr else ""
            primitive_fit_kind = (prim or {}).get("fit_kind", mr.get("primitive_fit_kind", "") if mr else "")
            primitive_fit_rel_err = (prim or {}).get("rec_err", mr.get("primitive_fit_rel_err", "") if mr else "")
            uses_target_mined = str((prim or {}).get("uses_target_mined_atoms", mr.get("uses_target_mined_atoms", False) if mr else False)) == "True"
            ...
            interpretation_trusted = str(mr.get("interpretation_trusted", False) if mr else False) == "True"
            tensor_path = mr.get("tensor_path", "") if mr else ""
            fail_reason = "" if covered else "conv1d_decoder_missing_or_rejected"
        ...
        elif leaf_param_name == "cls_token" or param_name.endswith("cls_token"):
            covered = True
            decoder_row_id = f"{owner_module_name}.cls_token"
            decoder_kind = "EXACT_CONSTANT_PARAMETER"
            exact_target_kind = "learned_class_token_constant"
            exact_err = 0.0
            primitive_fit_kind = "CONSTANT_PARAMETER"
            interpretation_trusted = True
            fail_reason = ""
        elif owner_module is not None and owner_module.__class__.__name__ == "Block" and leaf_param_name in ("c_attn", "w_resid"):
            covered = True
            decoder_row_id = f"{owner_module_name}.{leaf_param_name}"
            decoder_kind = "EXACT_CONSTANT_PARAMETER"
            exact_target_kind = "block_direct_attention_residual_gate_parameter"
            exact_err = 0.0
            primitive_fit_kind = "CONSTANT_PARAMETER"
            interpretation_trusted = True
            fail_reason = ""
...
def patch_module_rows_from_parameter_coverage(module_rows: List[Dict[str, Any]], parameter_coverage_rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for r in module_rows:
        ...
        if params and all(str(p.get("covered_by_decoder")) == "True" for p in params) and rr.get("decoder_kind") == "UNCOVERED":
            if rr.get("module_type") in ("Block", "ParticleTransformer") or "direct_parameters" in str(rr.get("fail_reason", "")):
                rr["decoder_kind"] = "STRUCTURAL_NOT_EXPLICITLY_TRACED"
                rr["accepted_exact"] = False
                rr["fail_reason"] = "direct_parameters_covered_but_container_graph_not_replayed"
                rr["honesty_note"] = "Direct parameters are covered as exact constants; Python/container forward graph is still Stage 2."
            else:
...
def build_matrix_coverage(module_rows: List[Dict[str, Any]], head_rows: List[Dict[str, Any]], linear_rows: List[Dict[str, Any]], dynamic_rows: List[Dict[str, Any]], primitive_rows: List[Dict[str, Any]], decoded_summary: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for r in module_rows:
        if r.get("module_type") == "Conv1d":
            mod = r["module_name"]
            prim = prim_by_record.get(f"{mod}.Conv1dW", {})
            try:
                ej = json.loads(r.get("extra_json", "{}"))
                w_shape = "x".join(map(str, ej.get("W_shape", [])))
                m_shape = "x".join(map(str, ej.get("matrix_for_fit_shape", [])))
            except Exception:
                w_shape = ""
                m_shape = ""
            rows.append({
                "matrix_id": f"{mod}.Conv1d.W",
                "module_name": mod,
                "module_type": "Conv1d",
                "matrix_kind": "Conv1d.W",
                "shape": w_shape,
                "exact_target_kind": "conv1d_exact_convolution_operator",
                "exact_reconstruction_error": r.get("reconstruction_error", ""),
                "accepted_exact": r.get("accepted_exact", False),
                "primitive_fit_kind": prim.get("fit_kind", r.get("primitive_fit_kind", "")),
                "primitive_fit_rel_err": prim.get("rec_err", r.get("primitive_fit_rel_err", "")),
            ...
            if m_shape:
                rows.append({
                    "matrix_id": f"{mod}.Conv1d.W_1x1_matrix",
                    "module_name": mod,
                    "module_type": "Conv1d",
                    "matrix_kind": "Conv1d.W_1x1_matrix",
                    "shape": m_shape,
                    "exact_target_kind": "conv1d_1x1_channel_matrix",
                    "exact_reconstruction_error": r.get("reconstruction_error", ""),
                    "accepted_exact": r.get("accepted_exact", False),
                    "primitive_fit_kind": prim.get("fit_kind", r.get("primitive_fit_kind", "")),
                    "primitive_fit_rel_err": prim.get("rec_err", r.get("primitive_fit_rel_err", "")),
                    ...
                    "uses_shared_mined_atoms": prim.get("uses_shared_mined_atoms", r.get("uses_shared_mined_atoms", False)),
                    "tensor_path": r.get("tensor_path", ""),
                    "honesty_note": "1x1 Conv1d exposed as channel matrix for primitive fitting.",
                })
...
def apply_matrix_coverage_status(model: nn.Module, decoded: Dict[str, Any], inventory_rows_: List[Dict[str, Any]], params_rows_: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # Add direct constant parameters (cls_token, c_attn, w_resid, etc.) as
    # explicit matrix/parameter targets. They are exact constants, but they are
    # not primitive-program explanations and do not close container graph replay.
    for p in param_cov:
        if p.get("decoder_kind") == "EXACT_CONSTANT_PARAMETER" and str(p.get("covered_by_decoder")) == "True":
            matrix_cov.append({
                "matrix_id": p.get("param_name", ""),
                "module_name": p.get("module_name", ""),
                "module_type": p.get("module_type", ""),
                "matrix_kind": "ExactConstantParameter",
                "shape": p.get("param_shape", p.get("shape", "")),
                "exact_target_kind": p.get("exact_target_kind", "constant_parameter"),
                "exact_reconstruction_error": p.get("exact_reconstruction_error", 0.0),
                "accepted_exact": True,
                "primitive_fit_kind": p.get("primitive_fit_kind", "CONSTANT_PARAMETER"),
                "primitive_fit_rel_err": "",
                "primitive_fit_quality": "CONSTANT_PARAMETER_NOT_PRIMITIVE_EXPLANATION",
                "uses_target_mined_atoms": False,
                "uses_shared_mined_atoms": False,
                "tensor_path": p.get("tensor_path", ""),
                "honesty_note": "Exact learned constant parameter target; container forward graph is still Stage 2.",
            })
    ...
    conv1d_rows = [r for r in decoded["module_rows"] if r.get("module_type") == "Conv1d"]
    ...
    all_conv1d_modules_covered = all(str(r.get("accepted_exact")) == "True" for r in conv1d_rows)
    ...
    matrix_coverage_closed = (
        ...
        and all_conv1d_modules_covered
    ...
    decoded["summary"].update({
        ...
        "conv1d_modules": len(conv1d_rows),
        "accepted_conv1d_modules": sum(1 for r in conv1d_rows if str(r.get("accepted_exact")) == "True"),
        "all_conv1d_modules_covered": all_conv1d_modules_covered,
...
def write_report(path: str, decoded: Dict[str, Any], inv: List[Dict[str, Any]], params: List[Dict[str, Any]], args: argparse.Namespace):
    s = decoded["summary"]
    md = []
    md.append("# PART_FULL_MODEL_MATRIX_COVERAGE_V1_4_6\n\n")
...
def self_test():
    ...
    assert shared_info.get("heldout_gate") == "real_base_plus_shared_decode_gain_grouped_by_shape", shared_info
    assert any(float(r.get("gain_heldout", 0.0)) > 0.0 for r in shared_rows), shared_rows

    # Mixed residual dimensions must not crash shared mining.
    # Real ParT produces different matrix sizes, e.g. 64x64 and 128x128.
    R8 = torch.eye(8) * 0.2
    R16 = torch.eye(16) * 0.2
    mixed_recs = (
        [{"record_id": f"mix8_{i}", "matrix": R8 + 0.001 * torch.randn(8, 8), "residual_after_base": R8 + 0.001 * torch.randn(8, 8)} for i in range(3)]
        + [{"record_id": f"mix16_{i}", "matrix": R16 + 0.001 * torch.randn(16, 16), "residual_after_base": R16 + 0.001 * torch.randn(16, 16)} for i in range(3)]
    )
    mixed_rows, mixed_info = mine_shared_primitives(mixed_recs, min_cluster_size=2, threshold=0.80, gain_threshold=0.001, decode_gain_threshold=0.001, max_atoms=8, ridge=1e-4)
    assert "8x8" in mixed_info.get("shape_groups", {}), mixed_info
    assert "16x16" in mixed_info.get("shape_groups", {}), mixed_info
    assert mixed_info.get("clusters", 0) >= 1, mixed_info

    # Conv1d with non-zero padding_mode must be replayed exactly by pre-padding.
    conv_reflect = nn.Conv1d(3, 4, kernel_size=3, padding=1, padding_mode="reflect").eval()
    capc = ModuleCapture(); capc.attach(conv_reflect)
    with torch.no_grad():
        _ = conv_reflect(torch.randn(2, 3, 9))
    capc.close()
    rowc, _, _ = decode_conv1d("conv_reflect", conv_reflect, capc.records[""], Path("/tmp/no"), Args()) if "" in capc.records else (None, [], [])
    if rowc is None:
        # Root modules are not hook-captured by name; call decoder directly with manual record.
        xconv = torch.randn(2, 3, 9)
        with torch.no_grad(): yconv = conv_reflect(xconv)
        rowc, _, _ = decode_conv1d("conv_reflect", conv_reflect, {"input": xconv, "output": yconv, "input_shape": "2x3x9", "output_shape": shape_str(yconv)}, Path("/tmp/no"), Args())
    assert rowc["accepted_exact"] is True, rowc
    assert "padding_mode" in rowc.get("extra_json", ""), rowc
...
def self_test_coverage():
    ...
    assert all("shape" in r and r["shape"] for r in dec["parameter_coverage_rows"]), dec["parameter_coverage_rows"]
    assert dec["summary"].get("ok") is False, dec["summary"]
    assert dec["summary"].get("stage1_ok") is True, dec["summary"]
    ...
    # Known direct constants such as cls_token must be covered as exact constant
    # parameter targets and must also appear in matrix coverage.
    class ClsTokenToy(nn.Module):
        def __init__(self):
            super().__init__()
            self.cls_token = nn.Parameter(torch.zeros(1, 1, 5))
            self.proj = nn.Linear(5, 2)
        def forward(self, x):
            cls = self.cls_token.expand(x.shape[0], -1, -1)
            return self.proj(cls + x[:, :1, :])
    cls_model = ClsTokenToy().eval()
    xcls = torch.randn(3, 4, 5)
    cap = ModuleCapture(); cap.attach(cls_model)
    with torch.no_grad():
        _ = cls_model(xcls)
    cap.close()
    inv_cls, params_cls, _ = inventory_rows(cls_model)
    dec_cls = decode_model(cls_model, cap, [], Args())
    dec_cls = apply_matrix_coverage_status(cls_model, dec_cls, inv_cls, params_cls)
    assert dec_cls["summary"]["matrix_coverage_status"] == "FULL_MODEL_MATRIX_COVERAGE_CLOSED", dec_cls["summary"]
    assert any(r["param_name"] == "cls_token" and r["covered_by_decoder"] for r in dec_cls["parameter_coverage_rows"]), dec_cls["parameter_coverage_rows"]
    assert any(r.get("matrix_kind") == "ExactConstantParameter" for r in dec_cls["matrix_coverage_rows"]), dec_cls["matrix_coverage_rows"]
...
def main():
    ...
    ap.add_argument("--tensor-dir", default="artifacts/latest/part_full_model_matrix_coverage_v1_4_6")
    ap.add_argument("--out-md", default="reports/latest/PART_FULL_MODEL_MATRIX_COVERAGE_V1_4_6.md")
    ap.add_argument("--out-json", default="manifests/latest/part_full_model_matrix_coverage_v1_4_6.json")
    ap.add_argument("--out-inventory", default="reports/latest/tables/part_full_model_inventory_v1_4_6.csv")
    ap.add_argument("--out-inventory-json", default="manifests/latest/part_full_model_inventory_v1_4_6.json")
    ap.add_argument("--out-parameters", default="reports/latest/tables/part_full_model_parameters_v1_4_6.csv")
    ap.add_argument("--out-modules", default="reports/latest/tables/part_full_model_modules_v1_4_6.csv")
    ap.add_argument("--out-heads", default="reports/latest/tables/part_full_model_attention_heads_v1_4_6.csv")
    ap.add_argument("--out-linear", default="reports/latest/tables/part_full_model_linear_v1_4_6.csv")
    ap.add_argument("--out-dynamic", default="reports/latest/tables/part_full_model_dynamic_ops_v1_4_6.csv")
    ap.add_argument("--out-primitive", default="reports/latest/tables/part_full_model_primitive_fits_v1_4_6.csv")
    ap.add_argument("--out-shared", default="reports/latest/tables/part_full_model_shared_primitives_v1_4_6.csv")
    ap.add_argument("--out-uncovered", default="reports/latest/tables/part_full_model_uncovered_v1_4_6.csv")
    ap.add_argument("--out-pair-bias-candidates", default="reports/latest/tables/part_full_model_pair_bias_candidates_v1_4_6.csv")
    ap.add_argument("--out-acceptance", default="reports/latest/tables/part_full_model_acceptance_v1_4_6.csv")
    ap.add_argument("--out-flow", default="reports/latest/tables/part_full_model_flow_sketch_v1_4_6.csv")
    ap.add_argument("--out-parameter-coverage", default="reports/latest/tables/part_full_model_parameter_coverage_v1_4_6.csv")
    ap.add_argument("--out-matrix-coverage", default="reports/latest/tables/part_full_model_matrix_coverage_v1_4_6.csv")
    ap.add_argument("--out-layer-atlas", default="reports/latest/tables/part_full_model_layer_matrix_atlas_v1_4_6.csv")
    ap.add_argument("--out-pair-bias-audit", default="reports/latest/tables/part_pair_bias_producer_audit_v1_4_6.csv")
    args = ap.parse_args()
