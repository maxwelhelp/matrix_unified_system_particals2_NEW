...
# -*- coding: utf-8 -*-
"""
qwen_pseudocode_final_product_v5_no_na_fixed.py
...
Usage examples:
  python qwen_pseudocode_final_product_v5_no_na_fixed.py \
...
def row_get_metric(r: Dict[str, str], base: str) -> Any:
    """Get a metric from row supporting raw and aggregated atlas column names.

    Runtime atlas uses columns like score_rel_mean/Y_rel_mean because each row is
    already aggregated over prompts for one head. Older reports expected score_rel.
    """
    candidates = [base, base + "_mean", base + "_median", base + "_med"]
    # Some older files use lower-case variants or KL names.
...
def row_head_name(r: Dict[str, str]) -> str:
    if r.get("head") and str(r.get("head")).startswith("L"):
        return str(r.get("head"))
    layer = row_get_metric(r, "layer") or r.get("layer") or r.get("layer_idx") or r.get("layer_mean")
    head = r.get("head") or r.get("head_idx") or r.get("head_mean")
    try:
        layer_i = int(float(layer))
    except Exception:
        layer_i = layer if layer is not None else "?"
    try:
        head_i = int(float(head))
    except Exception:
        head_i = head if head is not None else "?"
    return f"L{layer_i}H{head_i}"
...
def summarize_runtime(atlas: Optional[Path]) -> Dict[str, Any]:
    ...
    for c in cols:
        vals = [fnum(row_get_metric(r, c)) for r in rows]
    ...
    for c in ["no_k_Y_rel", "no_content_Y_rel", "no_vo_bias_Y_rel"]:
        ranked = sorted(rows, key=lambda r: fnum(row_get_metric(r, c), -1) or -1, reverse=True)[:10]
...
def compact_head_row(r: Dict[str, str], metric: str) -> Dict[str, Any]:
    return {
        "head": row_head_name(r),
        metric: fmt(row_get_metric(r, metric)),
        "score_rel": fmt(row_get_metric(r, "score_rel")),
        "A_rel": fmt(row_get_metric(r, "A_rel")),
        "Y_rel": fmt(row_get_metric(r, "Y_rel")),
    }
