...
def main():
    ...
    ap.add_argument('--checkpoint',default='local_checkpoints/part/ParT_kinpid.pt')
    ...
    ap.add_argument('--out-dir',default='runs/particle_real_patch_controls_v3_official')
    a=ap.parse_args(); out=Path(a.out_dir); (out/'tables').mkdir(parents=True,exist_ok=True)
    from weaver.nn.model.ParticleTransformer import ParticleTransformer
    mode=mode_from_ckpt(a.checkpoint)
    model=ParticleTransformer(**cfg_for(mode)).to(a.device).eval()
    res=model.load_state_dict(clean(unwrap(torch.load(a.checkpoint,map_location='cpu'))),strict=False)
    batch=load_jetclass_balanced_official(a.data_dir,mode=mode,samples_per_file=a.samples_per_file,max_files=a.max_files,device=a.device)
    ad=ParticleTransformerAdapter(model); base=logits(model,batch); y=batch['y']; pred=base.argmax(-1)
    rows=[]
    ...
    wcsv(out/'tables/real_particle_patch_controls_v3_official.csv',rows)
    summary={'ok':True,'checkpoint':a.checkpoint,'mode':mode,'preprocess':'official_wrap_v3','missing':list(res.missing_keys),'unexpected':list(res.unexpected_keys),'n':int(y.numel()),'files':batch['files'],'labels':LABELS,'baseline':rows[0],'top_patches':rows_sorted[:30]}
    wjson(out/'real_particle_patch_v3_official_summary.json',summary)
    md=['# Real Particle Patch Controls v3 official preprocess\n\n',f"checkpoint={a.checkpoint}\nmode={mode}\npreprocess=official_wrap_v3\nn={summary['n']} samples_per_file={a.samples_per_file}\nmissing={summary['missing']} unexpected={summary['unexpected']}\n\n",'## Baseline\n',json.dumps(rows[0],indent=2,ensure_ascii=False),'\n\n## Top patches\n',table(['rank','patch','layer','group','delta_logit','KL','top1','acc->patch_acc'],[[i+1,r['patch'],r.get('layer',''),r.get('group',''),fmt(r['delta_pred_logit']),fmt(r['kl']),fmt(r['top1_match']),f"{fmt(r['base_acc'])}->{fmt(r['patch_acc'])}"] for i,r in enumerate(rows_sorted[:40])])]
    (out/'REAL_PARTICLE_PATCH_V3_OFFICIAL_REPORT.md').write_text(''.join(md),encoding='utf-8')
